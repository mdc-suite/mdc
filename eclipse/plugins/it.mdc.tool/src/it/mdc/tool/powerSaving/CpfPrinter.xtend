...
class CpfPrinter {
	...
	/** Estimate number of isolation cells 
	...
	def printCPF(Network network, 
		...
		define_always_on_cell -cells {PBUFX2 PINVX1} -power_switchable VDD -power ExtVDD -ground VSS
		
		
		# define the state retention cell
		define_state_retention_cell -cells {RDFF* SRDFF*} -ground VSS -power ExtVDD -power_switchable VDD -save_function !RT -restore_function RT -always_on_pins {RT} -always_on_components {ff}
