...
public class Searcher {
  ...
  public Vertex findBestVertex(Vertex candidate, List<Vertex> sharables) {
    ...
    // searching matching input connections of the vertex in the network with
    // the input connections of the candidate
    for (Vertex unifiable : sharables) {
      matchingConnections = 0;
      for (Connection connection :
           candidate.getAdapter(Instance.class).getIncomingPortMap().values())
        for (Connection unifiableConnection :
             unifiable.getAdapter(Instance.class)
                 .getIncomingPortMap()
                 .values()) {

          if (matcher.matchConnectionSboxMultiple(
                  connection, unifiableConnection, INIT_SEARCH)) {
            matchingConnections += 1;
            break;
          }
        }
      // searching matching output connections of the vertex in the network with
      // the output connections of the candidate
      for (List<Connection> connectionList :
           candidate.getAdapter(Instance.class).getOutgoingPortMap().values())
        for (Connection connection : connectionList)
          for (List<Connection> unifiableConnectionList :
               unifiable.getAdapter(Instance.class)
                   .getOutgoingPortMap()
                   .values())
      ...
      OrccLogger.traceln("vertex " + unifiable + " match conn " +
                         matchingConnections);
    }
    return bestVertex;
  }
   ...
   */
  public Connection getConnection(Vertex source, Port sourcePort, Vertex target,
                                  Port targetPort, Network network,
                                  int broadcastSize) {

    Map<Connection, Map<Instance, Boolean>> existingConnections =
        new HashMap<Connection, Map<Instance, Boolean>>();
    ...
    for (Connection existingConnection : network.getConnections()) {
      ...
      if (matcher.matchConnectionSboxCompl(source, sourcePort, target,
                                           targetPort, existingConnection,
                                           INIT_SEARCH))
   ...
   */
  public Connection searchExistingConnection(Connection candidate,
                                             Network network) {
   ...
   * 		the existing sharable connection
   */
  public Connection searchExistingConnectionMultiple(Connection candidate,
                                                     Network network,
                                                     List<SboxLut> luts,
    ...
    if (!(candidate == null))
      for (Connection existing : network.getConnections())
        if (matcher.matchConnectionSboxMultiple(candidate, existing,
                                                INIT_SEARCH))
  ...
  public Vertex searchExistingVertex(Vertex candidate, Network network) {
    ...
    if (candidate.getAdapter(Port.class) != null) {
      ...
      for (Instance instance :
           network.getInstancesOf(candidate.getAdapter(Actor.class)))
   ...
   * 		the existing sharable vertex
   */
  public Vertex searchExistingVertexMultiple(Vertex candidate,
                                             Network network) {

    // vertex is a port
    if (candidate.getAdapter(Port.class) != null) {
      for (Port nextInput : network.getInputs()) {
        if (unifier.canUnifyMultiple(candidate.getAdapter(Port.class),
                                     nextInput)) {
          return nextInput;
        }
      }
   ...
   */
  public Set<Instance> searchSameInstanceSet(Set<Instance> firstSet,
                                             Set<Instance> secondSet) {
